...
bool plan_transfers(
		...
		for (uint32_t i = 0; i < cycle.size(); ++i) {
			...
			if (cycle[i].bed == BedNeedle::Front) {
				if (cycle[n].bed == BedNeedle::Front) {
					...
					assert(cycle[n].bed == BedNeedle::Back);
					assert(!has_front_back);
					has_front_back = true;
				}
			}
			else {
				assert(cycle[i].bed == BedNeedle::Back);
				if (cycle[n].bed == BedNeedle::Back) {
					...
					assert(cycle[n].bed == BedNeedle::Front);
					assert(!has_back_front);
					has_back_front = true;
				}
			}
	...
	for (uint32_t i = 0; i < Count; ++i) {
		if (from[i].bed == BedNeedle::Front) {
			...
			assert(from[i].bed == BedNeedle::Back);
