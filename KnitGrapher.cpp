...
) 
{
	...
	qDebug() << "Found " << row_pending.size() << " rows.";
	...
	auto trace_yarn = [&]() -> bool {
		...
		auto rule4 = [&]() -> bool {
			...
			if (down_next != -1U && is_covered(down_next)) {
				qDebug() << "NOTE: not tucking in rule4 because down_next is covered.";
				down_next = -1U;
			}
			...
			if (down_next != -1U && info[down_next].knits == 2 && vertices[down_next].col_out[0] != -1U && vertices[down_next].col_out[1] != -1U) {
				...
				qDebug() << "NOTE: tucking on child of down_next because of increase.";
			}
			if (down_next != -1U && info[down_next].last_stitch != -1U && traced[info[down_next].last_stitch].type == TracedStitch::End) {
				qDebug() << "NOTE: not tucking on down_next because it is an end.";
				down_next = -1U;
			}

			if (down_next != -1U) {
				qDebug() << "  TUCKING[4] at " << down_next << " which has " << info[down_next].knits << " knits and outs " << int32_t(vertices[down_next].col_out[0]) << " and " << int32_t(vertices[down_next].col_out[1]);
			}
